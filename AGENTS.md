...
**Step 2.** Query the live NN at `nn.boonyard.com` (this repo's own node, `node/journal.db`, live since 2026-07-18 / M8). ⚠ CORRECTED 2026-08-25 (umbrella #228): this step named `nn.vectorscape.uk/sse` for a month after that wall was deregistered on 2026-07-26 — booting seats were sent to a dead endpoint, which is why this node went 26 days silent. **Read tools: `search_text`, `search_by_tag`, `get_thread`, `by_id`, `recent`, `upcoming_dates`. THE READ LAW: search before you assert; cite the entry id or say `NN: searched "<terms>" — no hit`.** Run:
```
recent(limit=20)
...
- The live NN at `nn.boonyard.com` (this repo's own node, `node/journal.db`, live since 2026-07-18 / M8) — the authoritative source for any conversation that happened in chat. (Was `nn.vectorscape.uk/sse`; corrected 2026-08-25, umbrella #228.)
