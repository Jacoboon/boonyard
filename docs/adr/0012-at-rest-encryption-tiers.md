...
> **2026-09-07 — clarification (umbrella #373; the order's §0.1, executed in boonyard #139).** "Restore-proof runs on the encrypted bundle" asks the box to decrypt what it wrote, which the same sentence forbids — the private half is not on the droplet, so the droplet cannot. What runs nightly: `verify_restore` on the **plaintext** immediately before encryption, sha256 of those same proven bytes, `age`, sha256 of the ciphertext, then the plaintext is deleted; the heartbeat carries both hash prefixes and the `.age` byte count. The ciphertext is proven by an **off-box decrypt-and-restore drill** where the private key lives, never on the server. A bundle that cannot be encrypted is kept as plaintext, the run exits PARTIAL, and the heartbeat says `UNENCRYPTED — plaintext kept`: never zero bundles, never silent plaintext.
...
> **2026-09-07 — clarification (umbrella #373; the order's §0.2).** Verified, and the answer changes the design: DigitalOcean's automated droplet backups do **not** include Volumes ("To back up a volume, create a volume snapshot"), while Volumes and their snapshots are LUKS-encrypted at rest. So 1b as written above would have moved the walls out of the only provider-side copy. Tier 1b therefore ships **paired with a ciphertext mirror on the root disk** — `/var/backups/boonyard/<slug>/`, `.age` bundles only, the same 7-deep rotation — so the provider's 7-day backup carries seven nights of ciphertext per node and zero plaintext. Durability kept, privacy gained; rung 4 (#270) is still the true offsite.
